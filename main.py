...
def draw(draw_info, algo_name, ascending):
    ...
    title = draw_info.LARGE_FONT.render(f"{algo_name} - {'Ascending' if ascending else 'Decending'}", 1, draw_info.GREEN)
    draw_info.window.blit(title, (draw_info.width/2 - title.get_width()/2,5))
    
    controls = draw_info.FONT.render("R - Reset | SPACE - Start Sorting | A - Ascending | D - Decending", 1, draw_info.WHITE)
...
#     lst = draw_info.lst
#     def helper(lst, ascending):
#         if len(lst) <= 1:
#             return lst
#         mid = len(lst) // 2
#         left, right = helper(lst[:mid], ascending), helper(lst[mid:], ascending)
        
        
        
#         return merge(left, right, lst)
#     return helper(lst, ascending)
